...
- 从 v2.8.x 更新到 v3 时，插件会在首次启动自动备份并迁移旧签到资料、偏好和主题购买；只有迁移校验成功后才删除旧数据表，失败时会回滚并保留原数据。
- 签到卡片 JPEG 缓存由签到缓存模块按天自行过期，重启时不会被整目录清空，当天重复签到仍可复用。日常启动不会删除数据库、黑名单和备份；仅在管理员明确导入新版 SQLite 数据库时永久替换旧签到数据库。
