...
签到业务只保留 `/签到` 和 `/签到中心` 两个顶层入口；纯文本“签到”继续由正则触发。只有根指令 `/签到中心` 时发送内置帮助图；带后续路径时进入 AstrBot 原生嵌套指令组，按“我的、排行、商店、管理”组织功能。旧平铺指令和独立的 `/签到帮助` 入口不再保留。根指令与指令组使用互斥过滤器，防止 AstrBot 在根指令阶段先发送“参数不足”的文字树。指令装饰器集中在 `main.py`，处理函数继续委托给 `CheckinCommandMixin` 与 `CheckinShopMixin`，避免命令结构与业务实现相互耦合。
