...
| `/签到生日 [设置 MM-DD|清除|同步]` | 查看或管理签到生日 | `/签到生日 设置 07-11` |
...
签到卡片固定渲染为 `960 × 540` 的 H 纸张画册：左侧展示身份、问候、今日奖励和成长进度，右侧使用固定 `3:4` 相框。Pixiv 作品按 `3:4 ± 20%` 严格筛选，只接受宽高比 `0.60–0.90`；当前页无匹配时继续翻页，不会改用横图或方图。作品下载失败、损坏或所有分页都无匹配时，相框显示内置纸张占位图，图片始终使用 `object-fit: contain` 完整展示，不裁切原图。
...
生日只保存月日，不保存出生年份。首次签到会在 OneBot QQ 平台尝试一次资料读取，失败后不会自动反复请求；用户可手动设置或执行同步。管理员可以添加插件全局年度事件和单次事件。主事件按“生日、单次事件、年度事件、节假日、签到里程碑、连续签到”排序，其他同日事件进入次要备注。
