...
- 今日运势（同一用户同一天在同一群聊/私聊中文本和配图固定，支持模型文案、Pixiv 配图、命令和纯文本触发）
- WebUI 图片历史：在 AstrBot 插件页面查看已成功发送的 Pixiv 图片记录和本地缩略图
...
| `今日运势` / `jrys` | 查看今日运势 | 同一用户同一天在同一群聊/私聊中文本和配图固定；默认尝试附带 Pixiv 配图，同群同天不同用户尽量不重复 |

## WebUI 图片历史

插件会在 AstrBot WebUI 的插件页面中提供「图片历史」页面，用于查看已经成功发送的 Pixiv 图片。记录内容包括标题、作者、作品 ID、页码、来源、R18 标记、尺寸、发送会话、发送时间、实际发送质量、文件大小和 Pixiv 链接。

图片历史只保存本地缩略图和元数据，不保留原始发送图片；原来的临时图片仍会在发送完成后按原生命周期清理。默认最多保留最近 200 条记录，超过后会同时删除对应旧缩略图。

图片历史只用于 WebUI 查看，和当天去重索引相互独立；清空历史不会清空当天已发作品索引。
...
| `dedupe_ttl_hours` | 大于 0 时启用普通发图当天去重：同群/私聊同标签或排行榜按北京时间当天记录已发作品，重启后仍有效，次日自动清理；设为 0 只关闭普通发图去重，今日运势仍按用户当天固定文本和配图 | 24 |
...
| `fortune_ai_text_enabled` | 今日运势模型文案；当天首次确定的文案会持久保存，后续请求和重启后不随模型/提示词配置变化 | 关闭 |
